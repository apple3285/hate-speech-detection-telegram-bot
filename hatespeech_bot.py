...
class DLBot(object):
    ...
    def activate_bot(self):
        ...
        self.updater = Updater("1378605484:AAEg82mw_sWFjygA_NIJ0uxqnyPSvLx0qos",use_context = True)  # setup updater
    ...
    def plot_loss(self,update,context):
        ...
        if check_hate == 0  :
            ...
            # pred_att
            # word_list = np.expand_dims(words, axis=0)
            # heatmap = sn.heatmap(pred_att, xticklabels=False, yticklabels=False,cbar=False, square=True,annot=word_list ,fmt ='', annot_kws={"alpha":1,'rotation':20},cmap ="coolwarm_r", linewidths=0.2, center=0.5.5.5.5.5, vmin=0, vmax=1)
            # plt.xticks(rotation=45)
            # plt.show()
            # buffer = BytesIO()
            # fig.savefig(buffer, format='png')
...
def main():
    telegram_token = "1378605484:AAEg82mw_sWFjygA_NIJ0uxqnyPSvLx0qos"  # replace TOKEN with your bot's token
